def worst_fit(program_size, memory, current_ptr):
    max_index = -1
    max_val = 0
    for i in range(len(memory)):
        if not memory[i][2] and memory[i][0] >= program_size and memory[i][0] - program_size > max_val:
            max_index = i
            max_val = memory[i][0] - program_size
    if max_index != -1:
        memory[max_index] = (memory[max_index][0] - program_size, program_size, True)
        return max_index
    return -1
...
for program_size in program_sizes:
    position = worst_fit(program_size, memory, current_ptr)
    if position == -1:
